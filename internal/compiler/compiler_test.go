// langur/compiler/compiler_test.go
...
func parse(t *testing.T, input string) *ast.Program {
	...
	checkParseErrors(t, input, p)
	return program
}

func checkParseErrors(t *testing.T, input string, p *parser.Parser) {
	t.Helper()
...
func runCompilerTests(
	...
	for _, tt := range tests {
		...
		err = compiler.CompileAnother(program)
		if err != nil {
...
func TestOpCodeSequencing(t *testing.T) {
	tests := []compilerTestCase{
		{
			// testing OpPop...
			input:             "1; 2",
...
func TestCallingCompiledFromBuiltIns(t *testing.T) {
	tests := []compilerTestCase{
		{
			input: `
				val x = fn(y) { y > 123 }
