...
type SymbolTable struct {
	...
	DefiningParams   bool     // when setting optional parameter defaults
	IsFunction       bool
...
func (st *SymbolTable) resolveSymbol(name string, fromLevel int) (
	...
	if st.DefiningParams {
		// symbol table in a temporarily unresolvable state
		// allows definition of full set of parameters before any can be resolved
		// (in case any default values might otherwise resolve other parameters)
		ok = false
	}

	if !ok && st.Outer != nil {
		// symbol not findable in current symbol table; check outer symbol table
